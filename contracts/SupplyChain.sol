  ...
  */
   ...
   */
    modifier forSale(uint256 sku) {
        require(items[sku].state == State.ForSale, "Not sold yet!");
        // require(bytes(items[sku].name).length > 0, "Not sold yet!");
        _;
    }
    modifier sold(uint256 sku) {
        require(items[sku].state == State.Sold, "Not sold yet!");
        _;
    }
    modifier shipped(uint256 sku) {
        require(items[sku].state == State.Shipped, "Not sold yet!");

        _;
    }
    modifier received(uint256 sku) {
        require(items[sku].state == State.Received, "Not sold yet!");
        _;
    }
